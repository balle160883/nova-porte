...
async function seedViajePrueba() {
  ...
  try {
    ...
    let conductorResult = await client.query(
    ...
    console.log('\n2. Verificando vehículo...');
    let vehiculoResult = await client.query(
    ...
    let rutaResult = await client.query(
